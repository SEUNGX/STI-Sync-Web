...
export function useOrgCollectionsStream(organizationId: string | null, semesterId?: string | null) {
  ...
  useEffect(() => {
    ...
    let currentPayables: PayableDocument[] = [];
    let activeMemberIds = new Set<string>();

    const recomputeGroups = () => {
      const groupsMap = new Map<string, StudentEventCollectionGroup>();

      for (const doc of currentPayables) {
        // For non-event club payables (membership dues, club fines, custom dues), strictly require that the student is an active member
        const isClubPayable = !doc.eventId || doc.type === 'membership_due' || doc.type === 'dues' || doc.type === 'custom' || doc.type === 'org_fine';
        if (isClubPayable && activeMemberIds.size > 0) {
          const sId = (doc.studentId || '').toLowerCase().trim();
          const schId = (doc.studentSchoolId || '').toLowerCase().trim();
          const isMember = (sId && activeMemberIds.has(sId)) || (schId && activeMemberIds.has(schId));
          if (!isMember) {
            continue;
          }
        }

        const cleanLabel = (doc.label || doc.feeTitle || doc.title || (doc.type === 'membership_due' ? 'Membership Dues' : 'Club Assessment')).trim();
        const groupId = doc.eventId
          ? `${doc.eventId}_${doc.type}`
          : `${doc.type || 'club_fee'}_${cleanLabel.toLowerCase()}_${doc.semesterId || 'all'}`;
        const eId = doc.eventId || 'unassigned';
    ...
    const unsubPayables = onSnapshot(
      qPayables,
      (snapshot) => {
        currentPayables = snapshot.docs.map((d) => ({
          id: d.id,
          ...d.data(),
        })) as PayableDocument[];
        recomputeGroups();
    ...
    const unsubMembers = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.docs.forEach((d) => {
          const m = d.data();
          if (m.studentId) set.add(String(m.studentId).toLowerCase().trim());
          if (m.studentSchoolId) set.add(String(m.studentSchoolId).toLowerCase().trim());
          if (d.id) set.add(d.id.toLowerCase().trim());
        });
        activeMemberIds = set;
        recomputeGroups();
      },
      (err) => {
        console.warn('Error fetching active members for collections:', err);
      }
    );
